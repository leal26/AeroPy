...
# Numpy module is imported in case one of the inputs is a numpy array and for
...
#                       	Core Functions
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def call(airfoil,alfas='none',output='Cp',Reynolds=0,Mach=0,plots=False,NACA=False,
         GDES=False,iteration=10):
    """ Call is a function that enables the use of xfoil through Python.
    The input variables are:
        - airfoil: if NACA is false, airfoil is the name of the plain file 
          where the airfoil geometry is stored (variable airfoil). If NACA is 
          True, airfoil is the naca series of the airfoil (i.e.: naca2244). By
          default NACA is False.
          
        - alfas: list/array/float/int of angles of attack.
        
        - output: defines the kind of output desired from xfoil. There are
          four posssible choices:
              - Cp: generates files with Pressure coefficients for desired
                alfas
              - Dump: generates file with Velocity along surface, Delta star,
                theta and Cf vs s,x,y for several alfas
              - Polar: generates file with CL, CD, CM, CDp, Top_Xtr, Bot_Xtr
        ...
        - Reynolds: Reynolds number in case the simulation is for a viscous 
          flow. In case not informed, the code will assume inviscid.
          
        - Mach: Mach number in case the simulation has to take in account 
          compressibility effects through the Prandtl-gauber correlation. If 
          not informed, the code will not use the correction. For logical 
          reasons, if Mach is informed a Reynolds number different from zero 
          must also be informed.
          
        - plots: the code is able to save in a .ps file all the plots of Cp vs.
          alfa. By default, this option is deactivated.
          
        - NACA: Boolean variable that defines if the code imports an airfoil 
          from a file or generates a NACA airfoil.
          
        - GDES: XFOIL function that improves the airfoil shape in case the 
          selected points do not provide a good shape. the CADD function is 
          use also. FOr more information about these function, look the help 
          files for XFOIL
          
        - iteration: changes how many times XFOIL will try to make the results
          converge. Speciallt important for viscous flows
          
    As a side note, it is much more eficient to run a single run with multiple
    angles of attack rather than multiple runs, each with a single angle of 
    attack
    ...
    def issueCmd(cmd,echo=True):
        """Function that submits commands through the PIPE to the command line,
    ...
    def submit(output,alfa):
        ...
        Standard output file= function_airfoil_alfa.txt, where alfa has 4 
        digits, where two of them are for decimals. i.e. cp_naca2244_0200. 
        Analysis for Pressure Coefficients for a naca2244 at an angle of 
        degrees.
        
        Possible to output other results such as theta, delta star through the
        choice of the ouput, but not implemented here."""
        if output=="Alfa_L_0":
			issueCmd('CL 0')
    ...
    """
    For communication with the xfoil through the command line the Popen class 
    from subprocess is used. stdin and stdout both represent inputs and outputs 
    with the process run on the command line, in this case, xfoil.
    ...
    if output=='Coordinates':
        ...
        # Once you load a set of points in Xfoil you need to create a name, for now
        # I am not giving it a name by inputing nothing
        issueCmd('')
        ...
        """
        The following part of the code was not implemented in hope that the Cp
        be given for the given airfoil points, not for points created by xfoil.    
        ...
        if output=='Polar' or output=='Alfa_L_0':
            issueCmd('PACC')
            # For naming tte file, the first and last used angles of attack are
            # annexed to the file
            filename=file_name(airfoil,alfas,output)
            # Name of file with polar information
            # issueCmd('{!s}'.format(filename))
            try:
                os.remove(filename)
            except OSError:
                pass
...
def create_x(c):
    """
    The ojective of this function is to create the set of points along the 
    chord. The x output is conviniently ordered from TE to LE. 
    
    Because Xfoil it is not efficient or sometimes possible to create a
    uniform distribution of points because the front part of the airfoil
    requires a big amount of points to create a smooth surface representing 
    the round leading edge. However there is a maximum of points that Xfoil
    will accept, therefore there is a need to overcome this obstacle. This 
    was done by dividing the airfoil in to 3 parts.
    
        - tip: correpondent to 0 to .3% of the chord length, it is the most 
          densily populated part of the airfoil to compensate the wide 
          variation of slopes
        
        - middle: correpondent to .3% to 30% of the chord length (Shortly above
          a quarter of the chord length). The second most densily populated and 
          the segment with the most amount of points. Represents the round
          section of the airfoil except for the tip. Such an amount of points 
          is necessary because it is where most of the lift is generated.
        
        - endbody: correspondent to 30% to 100% of the chord length. The less 
          densily populated section. Such unrefined mesh is possible because of
          the simplistic geometry of endbody's airfoil, just straight lines.
...
# TODO: merge create_inpt and prepare_xfoil
# prepare_xfoil is for disctionaries while create_input is for two separate
# lists. Create_input also creates the file while prepare_xfoil retuns a tuple

def create_input(x,y_u,y_l,filename):
    """
    Function generates a plain file that xfoil can read.
    
    XFOIL only reads file from the TE to the LE from the upper part first and 
    then from the LE to the TE through the pressure surface.
    ...
    # XFOIL likes to read the files from the TE to the LE from the upper part 
...
def prepare_xfoil(Coordinates_Upper,Coordinates_Lower,chord,reposition=False,FSI=False):
    # The upper and lower functions will be the points in ordered fashion. 
    # Because of the way that XFOIL works the points start at the Trailing Edge
    # on the upper surface going trough the Leading Edge and returning to the
    # Trailing Edge form the bottom surface
    def Reposition(CoordinatesU,CoordinatesL):
        ...
        # Find the coordinates of the trailing edge. Due to the thickness of
        # the TE, it is necessary to find the point with max(x) for both
        # surfaces and take the average between them to find the actual TE        
        TE={'x':0,'y':0}
    ...
    for i in range(len(Coordinates_Upper['x'])):
        # For each x value, we will check the correpondent y value so that we 
        # can classify them as upper or lower
        upper.append([Coordinates_Upper['x'][i]/chord,Coordinates_Upper['y'][i]/chord])

    for i in range(len(Coordinates_Lower['x'])):
        # For each x value, we will check the correpondent y value so that we 
        # can classify them as upper or lower
        lower.append([Coordinates_Lower['x'][i]/chord,Coordinates_Lower['y'][i]/chord])
    ...
    if reposition==True:
        ...
        # Sort in a way that comprehensible for xfoil and elimates the repeated
        # point at the LE
...
def output_reader(filename,separator='\t',output=None,rows_to_skip=0,header=0):
    """
    Function that opens files of any kind. Able to skip rows and read headers
    if necessary.
    
    Inputs:
        - filename: just the name of the file to read.
        
        - separator: Main kind of separator in file. The code will replace any
          variants of this separator for processing. Extra components such as 
          \n kg m are all eliminated
          
        - output: defines what the kind of file we are opening to ensure we
          can skip the right amount of lines. By default it is None so it can
          open any other file.
          
        - rows_to_skip: amount of rows to initialy skip in the file. If the
          output is different then None, for the different types of files it is
          defined as:
          - Polar files = 10
          - Dump files = 0
          - Cp files = 2
        ...
        - header: The header list will act as the keys of the output
          dictionary. For the function to work, a header IS necessary. If not 
          specified by the user, the function will assume that the header can
          be found in the file that it is opening
    Output:
    ...
    @author: Pedro Leal
    """
    # In case we are using an XFOIL file, we define the number of rows skipped
    if output=='Polar' or output=='Alfa_L_0':
        rows_to_skip=10
    elif output=='Dump':
        rows_to_skip=0
    ...
    with open (filename, "r") as myfile:
        ...
        for line in myfile:
            if count_skip<rows_to_skip:
                ...
                # If the user did not specify the header the code will read
                # the first line after the skipped rows as the header
                if header==0:
                    # Open line and replace anything we do not want( variants
                    # of the separator and units)
                    ...
                    for head in header:
                        Data[head]=[]
                    # To avoid having two headers, we assign the False value to
                    # header which will not let it happen
                    header_done=True
                # If the user specified a list of values for the header, the 
                # code reads it and creates libraries with it.
...
def file_name(airfoil,alfas='none',output='Cp'):
    ...
    The input variables are:
        - airfoil: the name of the plain file where the airfoil geometry is 
          stored (variable airfoil).
          
        - alfas: list/array/float/int of a single angle of attack for Cp
          and Dump, but the whole list for a Polar. Only the initial and the
          final values are used
        
        - output: defines the kind of output desired from xfoil. There are
          three posssible choices:
              - Cp: generates files with Pressure coefficients for desired
                alfas
              ...
              - Polar: generates file with CL, CD, CM, CDp, Top_Xtr,
                Bot_Xtr
			  - Alfa_L_0: calculate the angle of attack that lift is zero
    The output has the following format:
        - for Cp and Dump: output_airfoil_alfa
           i.e. Cp_naca2244_0200 (alfa =2.00 degress)
        - for Polar: Polar_airfoil_alfa_i_alfa_f
           i.e. Polar_naca2244_n0200_0200 (alfa_i=-2.00 degrees and alfa_f=2.00)
    ...
    if alfas=='none':
		...
		if output=='Cp' or output=='Dump':
			...
			#filename='{!s}_{!s}_{!s}'.format(output,airfoil,alfa)
			# To make it work in Abaqus
			filename='%s_%s_%s' % (output,airfoil,alfa)
				
		if output=='Polar':
			# In case it is only for one angle of attack, the same angle will be
			# repeated. This is done to keep the formating
			if type(alfas)==int or type(alfas)==float:
				alfas=[alfas]
			alfa_i=alfa_for_file(alfas[0])
			alfa_f=alfa_for_file(alfas[-1])
			# Name of file with polar information
			#filename='{!s}_{!s}_{!s}_{!s}'.format(output,airfoil,alfa_i,alfa_f)
...
def air_properties(height,unit='feet'):
    ...
    Sources: http://en.wikipedia.org/wiki/Density_of_air#Altitude
    ...
    # Temperature
    #==============================================================================
    #Temperature at altitude h meters above sea level is approximated by the 
    # following formula (only valid inside the troposphere):
...
def find_coefficients(airfoil,alpha,Reynolds=0,iteration=10,NACA=True):
    ...
    # TODO: need to include velocity in file name and take care of precision
...
def force_shell(Data,chord,half_span,height,Velocity,thickness=0,txt=False):
    # Height is in feet
    # If the Shell is an extrude, it needs to take in consideration that there
    # is a skin thickness outwards of the outer mold.
    # If the Shell is na planar, there is no need for such a consideration
