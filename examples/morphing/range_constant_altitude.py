...
def aircraft_range_varying_V(f_L, f_LD, AOA):
    def to_integrate(weight):
        ...
        def calculate_velocity(V):
            CL = f_L(AOA, V)
            span = 11
            chord_root = span/16.2
            return math.sqrt(weight/(.5*density*(span*chord_root)))
            
        velocity = scipy.optimize.fixed_point(calculate_velocity, 30)
...
f_LD = scipy.interpolate.interp2d(data.values[:,-5], data.values[:,-4], data.values[:,-1], kind='cubic')
f_L = scipy.interpolate.interp2d(data.values[:,-5], data.values[:,-4], data.values[:,-3], kind='cubic')

ranges = []
AOA = np.linspace(0, 12, 100)
for i in range(len(AOA)):
    range_i = aircraft_range_varying_V(f_L, f_LD, AOA[i])
    ranges.append(range_i)
    
plt.figure()
plt.plot(AOA, ranges, 'k', lw=2)
plt.xlim(0, 12)
