...
dY = (max(Y) - min(Y))
X0 = np.concatenate((X[:-1], X, X[1:]))
Y0 = np.concatenate((Y[:-1] - dY + .5, Y + .5, Y[1:] + 2*dY + .5))
Z0 = np.concatenate((Z[:-1], Z, Z[1:]))
A0, output0 = calculating_area(X0, Y0, Z0, [y_min], nx)
print('A0', A0)
# fig = plt.figure()
# ax = Axes3D(fig)
# ax.scatter(X, Y, Z, c='b')
# x, y, z = output0.reshape(nx*1, 3).T
...
X = np.concatenate((X[:-1], X+U1, X[1:]))
Y = np.concatenate((Y[:-1] - dY + .5, Y + U2 + .5, Y[1:] + dY + .5))
Z = np.concatenate((Z[:-1], Z + U3, Z[1:]))

loudness = calculate_loudness(lambda xx: calculate_radius(xx-12.5, X=X, Y=Y, Z=Z,
                                                          nx=nx, A0=A0))
print(loudness)
